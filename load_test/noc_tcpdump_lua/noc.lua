do
	...
	local msg_types = {
        ...
        [1] = "NOC_RELAY_MSG_QUERY_HOST_RESP", --NOC_RELAY_MSG_QUERY_HOST_RESP
        [2] = "RelayRegisterRequest",
        [3] = "RelayRegisterResponse",
        [4] = "RelayMsgConnectionRequest",
        [5] = "RelayMsgConnectionResponse",
        [6] = "NOC_RELAY_MSG_DATA_STREAM_IND",          --NOC_RELAY_MSG_DATA_STREAM_IND
        ...
        [13] = "Reserved_3",
        [14] = "Reserved_4",
        [19] = "NOC_RELAY_MSG_PING",              --echo request, between gateway and relay
        [20] = "NOC_RELAY_MSG_PONG",              --echo response, between gateway and relay
        ...
        [25] = "PKG_ALLOCATE_HOST_ID_REQ",       --PKG_ALLOCATE_HOST_ID_REQ
        [26] = "PKG_ALLOCATE_HOST_ID_RESP"       --PKG_ALLOCATE_HOST_ID_RESP
     }
	...
	-- host count(4 bytes)
	local f_noc_host_count = ProtoField.uint32("NOC.HostCount", "Host Count", base.HEX)
    ...
    -- add_f_noc_indication_type(1 bytes)
    local f_noc_indication_type = ProtoField.uint8("NOC.IndicationType", "Indication Type", base.DEC)
    -- add f_noc_indication_data (1 bytes)
    local f_noc_indication_data = ProtoField.uint8("NOC.IndicationData", "Indication Data", base.DEC)
    -- add f_noc_host_state (4 bygtes)
    local f_noc_host_state = ProtoField.uint32("NOC.HostState", "Host State", base.HEX)
    -- add f_noc_transaction_id (4 bypes)
    local f_noc_transaction_id = ProtoField.uint32("NOC.TransactionId", "Transaction Id", base.DEC)
	...
	PROTO_NOC.fields = {f_noc_version, f_noc_msg_type, f_noc_length, f_noc_target_host_id, 
		f_noc_host_id, f_noc_connection_id, f_noc_result, f_noc_indication_type, f_noc_indication_data,
        f_noc_host_state, f_noc_transaction_id, f_noc_host_count, f_noc_host_type, f_noc_data_type,
        f_noc_host_key_len }
	...
	local function noc_dissector(buf, pkt, root)
        ...
        elseif msg_type == 1 then -- NOC_RELAY_MSG_QUERY_HOST_RESP
        	pkt.cols.info = "NOC_RELAY_MSG_QUERY_HOST_RESP"
        ...
        elseif msg_type == 22 then --  NOC_RELAY_MSG_CONTROL_INDICATION
        	...
        	t:add(f_noc_target_host_id, buf(4,4))
        	t:add(f_noc_host_id, buf(8,4))
            t:add(f_noc_indication_type, buf(12,1))
            t:add(f_noc_indication_data, buf(13,1))
        	pkt.cols.info = "NOC_RELAY_MSG_CONTROL_INDICATION"

		elseif msg_type == 6 then -- NOC_RELAY_MSG_DATA_STREAM_IND
			if buf_len < 12 or msg_len < 8 then 
        		return false
        	end
        	t:add(f_noc_target_host_id, buf(4,4))
        	...
        	pkt.cols.info = "NOC_RELAY_MSG_DATA_STREAM_IND (QUIC over NOC-Relay; data stream indication, between gw and relay)"
        ...
        elseif msg_type == 19 then --NOC_RELAY_MSG_PING
            pkt.cols.info = "NOC_RELAY_MSG_PING"

        elseif msg_type == 20 then --NOC_RELAY_MSG_PONG
            pkt.cols.info = "NOC_RELAY_MSG_PONG"
		
        elseif msg_type == 25 then --NOC_RELAY_MSG_PONG
        	t:add(f_noc_host_id, buf(4,4))                
        	t:add(f_noc_host_state, buf(8,4))                
            pkt.cols.info = "NOC_RELAY_MSG_PONG"
        elseif msg_type == 26 then --PKG_ALLOCATE_HOST_ID_RESP
            t:add(f_noc_result, buf(4,4))
            t:add(f_noc_transaction_id, buf(8,4))
        	t:add(f_noc_host_id, buf(12,4))                
        	t:add(f_noc_host_count, buf(16,4))                
            pkt.cols.info = "PKG_ALLOCATE_HOST_ID_RESP"
	...
	tcp_encap_table:add(12300, PROTO_NOC) -- Xgw
	tcp_encap_table:add(12301, PROTO_NOC) -- Provision
